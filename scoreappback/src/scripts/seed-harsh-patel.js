...
async function seedHarshPatel() {
    try {
        await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/atpl_score');
        ...
        const matches = [
            {
                ...
                matchDetails: {
                    playerStats: {
                        // bowling & fielding summary (for analytics)
                    ...
                    instantScore: 78,
                    performanceGrade: 'A',
                    // Real-time enter metadata (derived from ballByBall / score entering in UI)
                    realTimeEnter: {
                        target: 156,
                        side: 'A',
                        current: { runs: 42, wickets: 1, balls: 32, overs: 2.5 },
                        // best-effort event extraction for analytics dashboard
                        events: {
                            wicketEvents: [
